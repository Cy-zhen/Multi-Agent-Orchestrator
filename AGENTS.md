...
- 删除任何文件前，先检查它是否被 Git 跟踪：
  - `git ls-files --error-unmatch <path>` 或 `git status --short <path>`
- 只有确认是未跟踪、且明确由本次操作生成的临时产物，才允许删除
- 不要仅凭目录名或扩展名推断“这一定是缓存”，例如 `__pycache__`、`.pyc`、截图、日志目录都可能被仓库跟踪
- 清理临时文件时，优先“先列出、再逐个删除”，不要按模式整目录粗暴清理
- 如果误删风险存在，先停下来检查 Git 状态，不要指望“删了再恢复”
...
- 在删除、清理、回滚任何文件前：
  - 先检查是否被 Git 跟踪
  - 不要把 `__pycache__`、`*.pyc`、截图、日志目录直接当成一定可删的临时产物
...
- 在删除、清理、回滚任何文件前：
  - 先检查是否被 Git 跟踪
  - 不要默认 `__pycache__`、`*.pyc`、截图目录、导出目录都可直接删除
- 如果当前项目存在 `doc/acceptance-contract.json`：
  - 在开始 FE 实现前先读它
