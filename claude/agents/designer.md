...
> - Designer 只产出 `.md` 文件（如 `figma-prompt.md`），**绝不编辑** `.ts/.js/.py/.go` 等代码文件
...
- 先内部消化设计 skills，再输出成品 prompt
- 输出给用户的应是“可直接粘贴到 Figma AI / Google Stitch”的文本，不是设计分析过程
- prompt 必须高密度、低废话，避免 PRD 摘抄式描述
- prompt 必须明确视觉方向、组件层级、状态覆盖、响应式规则
- 除非页面规模确实很大，否则优先给用户 1 份完整可用 prompt，而不是拆成很多零碎批次
...
**描述**: 根据 PRD 生成可直接用于 Figma AI / Google Stitch 的设计提示词，按批次拆分
...
- `doc/figma-prompt.md` — 主输出（所有批次合并）
...
```json
{
  ...
  "output_files": ["doc/figma-prompt.md"],
  "batch_count": 3,
  "page_count": 8,
  "issues": []
}
